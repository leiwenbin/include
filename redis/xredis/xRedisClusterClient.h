...
public:
    ...
    ~RedisResult() {
        if (Reply.reply) { freeReplyObject((void*) Reply.reply); }
    }
...
public:
    ...
    public:
        ...
        size_t len() const { return reply->len; }
...
private:
    ...
    bool ConnectRedisNode(uint32_t idx, const char* host, uint32_t port, uint32_t poolsize);
